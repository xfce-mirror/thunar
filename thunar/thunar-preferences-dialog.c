...
static void on_image_preview_full_changed        (GtkWidget                   *widget);
...
static void
on_image_preview_full_changed (GtkWidget *widget)
{
  ...
  for (GList *lp = windows; lp != NULL; lp = lp->next)
    thunar_window_image_preview_full_changed (lp->data);
...
thunar_preferences_dialog_init (ThunarPreferencesDialog *dialog)
{
  ...
  g_object_bind_property (G_OBJECT (dialog->preferences),
                          ...
                          "active",
                          G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
  g_signal_connect (button, "released", G_CALLBACK (on_image_preview_full_changed), NULL);
