...
thunar_preferences_dialog_init (ThunarPreferencesDialog *dialog)
{
  ...
  button = gtk_check_button_new_with_mnemonic (_("_Expandable folders in list view."));
  g_object_bind_property (G_OBJECT (dialog->preferences),
                          "misc-enable-expandable-folders",
                          G_OBJECT (button),
                          "active",
                          G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
  gtk_widget_set_tooltip_text (button, _("Select this option to open list-view on new tabs/windows with tree-view enabled."));
  gtk_widget_set_hexpand (button, TRUE);
  gtk_grid_attach (GTK_GRID (grid), button, 0, row, 1, 1);
  gtk_widget_show (button);
