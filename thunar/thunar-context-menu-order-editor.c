                                       ...
                                       gint                          index)
{
  ...
  XfceItemListModel               *item_view_model = xfce_item_list_view_get_model (item_view);
  GtkTreeIter                      iter;
  ...
  if (thunar_uca_editor_show (GTK_WINDOW (menu_editor), unique_id, NULL, THUNAR_UCA_EDITOR_SHOW_FLAG_NONE))
    {
      ...
      xfce_item_list_model_remove (item_view_model, index);
      thunar_context_menu_order_editor_insert_item (menu_editor, index, item);

      /* set cursor */
      xfce_item_list_model_set_index (item_view_model, &iter, index);
      path = gtk_tree_model_get_path (GTK_TREE_MODEL (item_view_model), &iter);
