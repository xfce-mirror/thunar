...
static gboolean
thunar_transfer_job_trash_file (ExoJob     *job,
                                GFileInfo  *info,
                                GFile      *target_parent,
                                GError    **error)
{
  ThunarJobResponse  response;
  GFileCopyFlags     flags = G_FILE_COPY_NOFOLLOW_SYMLINKS | G_FILE_COPY_NO_FALLBACK_FOR_MOVE | G_FILE_COPY_ALL_METADATA;
...
static gboolean
thunar_transfer_job_move_file_same_device (ExoJob         *job,
                                           GFile          *source_file,
                                           GFile          *target_file,
                                           GFileCopyFlags  flags,
                                           GError        **error)
{
  return g_file_move (source_file, target_file,
                      flags,
                      exo_job_get_cancellable (job),
                      NULL, NULL, error);
}
...
thunar_transfer_job_move_file_with_rename (ExoJob             *job,
                                           ...
                                           GError            **error)
{
  gboolean  move_rename_successful;
  gint      n_rename = 1;
  GFile    *renamed_file;

  node->rename_confirmed = TRUE;
  for (move_rename_successful = FALSE; !move_rename_successful; n_rename++)
    {
      g_clear_error (error);
      renamed_file = thunar_io_jobs_util_next_renamed_file (THUNAR_JOB (job),
                                                            node->source_file,
      ...
      if (*error == NULL)
        {
          ...
          move_rename_successful = thunar_transfer_job_move_file_same_device (job,
                                                                              node->source_file,
                                                                              renamed_file,
                                                                              flags, error);
        }
      if (!move_rename_successful)
        {
          if (!exo_job_is_cancelled (job) &&
              /* the error is because the target filesystem does not
               * handle direct move, usually this means a different
               * filesystem or the target is a directory. */
              (
          ...
          )
            {
              ...
              /* the rename (and copy) will take place in thunar_transfer_job_copy_file */
              break;
            }
        }
    }
                               ...
                               GError               **error)
{
  ...
  move_successful = thunar_transfer_job_move_file_same_device (job,
                                                               node->source_file,
                                                               tp->data,
                                                               flags, error);
      ...
      if (response == THUNAR_JOB_RESPONSE_REPLACE)
        {
          ...
          move_successful = thunar_transfer_job_move_file_same_device (job,
                                                                       node->source_file,
                                                                       tp->data, flags | G_FILE_COPY_OVERWRITE, error);
        }
      /* if the user chose to rename then try to do so */
      else if (response == THUNAR_JOB_RESPONSE_RENAME)
        {
          move_successful = thunar_transfer_job_move_file_with_rename (job, info, node, tp, flags, error);
        }
       ...
       sp = snext, tp = tnext)
    {
      ...
      if (transfer_job->type == THUNAR_TRANSFER_JOB_MOVE
          && thunar_g_file_is_trashed (node->source_file))
        {
          ...
          GFile    *target_parent = g_file_get_parent (tp->data);
          gboolean  job_running = thunar_transfer_job_trash_file (job, info, target_parent, &err);
          if (target_parent != NULL)
        ...
        {
          if (!thunar_transfer_job_move_file (job, info, sp, node, tp, thumbnail_cache, &new_files_list, &err))
            break;
        }
