...
thunar_transfer_job_freeze_optional (ThunarTransferJob *transfer_job)
{
  ...
  src_device_local = thunar_transfer_job_is_file_on_local_device_xfer (file);
  /* first target file */
  ...
  if (target_file != file)
    g_object_unref (target_file);
  tgt_device_local = thunar_transfer_job_is_file_on_local_device_xfer (file);
  ...
  if (!parallel_copy)
    {
          ...
          if (!thunar_job_is_frozen (THUNAR_JOB (transfer_job)))
            {
              if (been_frozen)
              ...
              else
                {
                  ...
                  been_frozen = TRUE;
                  thunar_job_set_frozen (THUNAR_JOB (transfer_job));
      ...
      if (thunar_job_is_frozen (THUNAR_JOB (transfer_job)))
        {
          thunar_job_unset_frozen (THUNAR_JOB (transfer_job));
