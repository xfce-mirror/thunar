                               ...
                               GError            **error)
{
  ...
  if (G_UNLIKELY (windows_reserved_name == NULL))
    {
      windows_reserved_name = g_regex_new ("^((COM\\d)|(LPT\\d)|(CON)|(PRN)|(AUX)|(NUL))(\\..*)?$", 0, 0, NULL);
    }
  ...
  for (; err == NULL && node != NULL; node = node->next)
    {
      ...
      /* https://gitlab.gnome.org/GNOME/nautilus/-/blob/619970b1e831daaca349c03373702dd63c400713/src/nautilus-file-operations.c#L4280 */
      if (use_fat_name_scheme)
        {
          ...
          g_strdelimit (g_strstrip (base_name),
                        "/:*?\"<>\\|",
                        '_');
