...
thunar_compact_view_init (ThunarCompactView *compact_view)
{
 ...
 /* initialize the icon view properties */
  ...
  g_object_get (G_OBJECT (THUNAR_STANDARD_VIEW (compact_view)->preferences), "misc-compact-view-homogenous-columns", &homogenous_columns, NULL);
  ...
  if (homogenous_columns == FALSE)
    {
      /* wrap only very long names */
      g_object_set (G_OBJECT (THUNAR_STANDARD_VIEW (compact_view)->name_renderer),
  ...
  else
    {
      g_object_set (G_OBJECT (THUNAR_STANDARD_VIEW (compact_view)->name_renderer),
                    ...
                    "width-chars", 50,
                    NULL);
      gtk_cell_renderer_set_fixed_size (GTK_CELL_RENDERER (THUNAR_STANDARD_VIEW (compact_view)->name_renderer), 150, -1);
    }
}
