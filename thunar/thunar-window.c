...
// TODO ^ keep old action name "cancel-search" to avoid keybind conflict?
...
thunar_window_update_search (ThunarWindow *window)
{
  ...
  g_free (window->search_query);
  window->search_query = thunar_location_bar_get_search_query (THUNAR_LOCATION_BAR (window->location_bar));
  thunar_standard_view_set_search_mode (THUNAR_STANDARD_VIEW (window->view), window->search_query);
...
thunar_window_reset_view_type_idle (gpointer window_ptr)
{
  ...
  /* null check for the same reason as thunar_standard_view_set_search_mode */
  if (window->view != NULL)
    {
...
thunar_window_cancel_search (ThunarWindow *window)
{
  ...
  thunar_standard_view_set_search_mode (THUNAR_STANDARD_VIEW (window->view), NULL);
