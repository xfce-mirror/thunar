...
thunar_window_location_toolbar_create (ThunarWindow *window)
{
  ...
  tool_item = GTK_TOOL_ITEM (xfce_gtk_toggle_tool_button_new_from_action_entry (get_action_entry (THUNAR_WINDOW_ACTION_VIEW_SPLIT), G_OBJECT (window), thunar_window_split_view_is_active (window), GTK_TOOLBAR (window->location_toolbar)));
  g_object_set_data_full (G_OBJECT (tool_item), "label", g_strdup (get_action_entry (THUNAR_WINDOW_ACTION_VIEW_SPLIT)->menu_item_label_text), g_free);
  g_object_set_data_full (G_OBJECT (tool_item), "icon", g_strdup (get_action_entry (THUNAR_WINDOW_ACTION_VIEW_SPLIT)->menu_item_icon_name), g_free);
  g_signal_connect (G_OBJECT (tool_item), "button-press-event", G_CALLBACK (thunar_window_toolbar_button_clicked), G_OBJECT (window));
