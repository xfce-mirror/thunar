...
thunar_window_set_current_directory (ThunarWindow *window,
                                     ThunarFile   *current_directory)
{
  ThunarStandardView *v         = NULL;
  ThunarDetailsView  *dv        = NULL;
  GFile              *folder    = NULL;
  ...
  v = THUNAR_STANDARD_VIEW (window->view);
  if (!THUNAR_IS_DETAILS_VIEW(v))
    return;
  dv = THUNAR_DETAILS_VIEW (v);

  folder = thunar_file_get_file (current_directory);
  thunar_details_view_set_date_deleted_column_visible (dv, thunar_g_file_is_trash(folder));
