...
static gboolean  thunar_window_action_gallery_view        (ThunarWindow           *window);
                                ...
                                GtkWidget    *menu)
{
  ...
  if (window->is_searching == TRUE)
    gtk_widget_set_sensitive (item, FALSE);
  xfce_gtk_toggle_menu_item_new_from_action_entry (get_action_entry (THUNAR_WINDOW_ACTION_VIEW_AS_GALLERY),
                                                   G_OBJECT (window), window->view_type == THUNAR_TYPE_GALLERY_VIEW, GTK_MENU_SHELL (menu));
...
static gboolean
thunar_window_action_gallery_view (ThunarWindow *window)
{
  if (window->is_searching == FALSE)
    thunar_window_action_view_changed (window, THUNAR_TYPE_GALLERY_VIEW);
