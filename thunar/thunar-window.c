                                               ...
                                               guint               item_order)
{
  ...
  icon_name = g_strjoin (NULL, get_action_entry (action)->menu_item_icon_name, "-symbolic", NULL);
  image = gtk_image_new_from_icon_name (icon_name, GTK_ICON_SIZE_LARGE_TOOLBAR);
  toolbar_item = gtk_tool_button_new (image, get_action_entry (action)->menu_item_label_text);
  g_signal_connect_swapped (G_OBJECT (toolbar_item), "clicked", get_action_entry (action)->callback, G_OBJECT (window));
  gtk_widget_set_tooltip_text (GTK_WIDGET (toolbar_item), get_action_entry (action)->menu_item_tooltip_text);
