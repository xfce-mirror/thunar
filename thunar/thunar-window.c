...
struct _ThunarWindow
{
  ...
  /* Per-tab terminal management */
  ThunarTerminalWidget *terminal_widget; /* Currently active terminal for the focused tab */
  GHashTable *view_terminals; /* Maps ThunarView widgets to their terminal widgets */
  GtkWidget *statusbar;
  ...
  gboolean suppress_terminal_sync; /* Prevent terminal sync on tab switch */
};
...
/**
 * on_tab_paned_size_allocated:
 * @paned: The GtkPaned widget for a tab.
 * @allocation: The new size allocation.
 * @user_data: The ThunarTerminalWidget associated with the paned.
 *
 * This callback is connected to the "size-allocate" signal of the tab's
 * GtkPaned. It is triggered when the paned receives its final size, ensuring
 * that the terminal's height can be set correctly. It disconnects itself
 * after the first run to avoid running on every subsequent resize.
 **/
static void
on_tab_paned_size_allocated (GtkWidget *paned, GtkAllocation *allocation, gpointer user_data)
{
  ...
  /* apply_new_size now correctly reads the latest height from settings */
                                   ...
                                   GtkWidget    *new_view)
{
  ...
  thunar_window_notebook_set_current_tab (window, gtk_notebook_page_num (GTK_NOTEBOOK (window->notebook_selected), gtk_widget_get_parent(new_view)));
                                    ...
                                    ThunarWindow *window)
{
  ...
  /* The page is now the container (a GtkBox) holding the paned view/terminal */
  view = g_object_get_data(G_OBJECT(page), "thunar-view");
                                     ...
                                     ThunarWindow *window)
{
  ...
  /* The 'page' is now the GtkPaned container. Get the view from its data. */
  view = g_object_get_data(G_OBJECT(page), "thunar-view");
  _thunar_return_if_fail (THUNAR_IS_VIEW (view));
                                           ...
                                           ThunarWindow   *window)
{
  ...
  if ((event->button == 2 || event->button == 3)
      && event->type == GDK_BUTTON_PRESS)
    {
      ...
      if (page == NULL)
        return FALSE;
      view_to_destroy = g_object_get_data(G_OBJECT(page), "thunar-view");
      ...
      if (event->button == 2)
        {
          ...
          if (close_tab)
            gtk_widget_destroy (view_to_destroy);
        }
                                    ...
                                    GtkWidget    *view)
{
  ...
  g_object_bind_property (G_OBJECT (spinner), "active", G_OBJECT (spinner), "visible", G_BINDING_SYNC_CREATE);
  g_object_bind_property (G_OBJECT (view), "searching", G_OBJECT (spinner), "active", G_BINDING_SYNC_CREATE);
  ...
  g_object_set_data (G_OBJECT (tab_content_paned), "thunar-view", view);

  g_signal_connect (tab_content_paned, "size-allocate",
                    G_CALLBACK (on_tab_paned_size_allocated), terminal);
                                    ...
                                    ThunarNewTabBehavior behavior)
{
  ...
  if ((behavior == THUNAR_NEW_TAB_BEHAVIOR_FOLLOW_PREFERENCE && switch_to_new_tab == TRUE)
    ...
    {
      GtkWidget* new_page = gtk_widget_get_parent(gtk_widget_get_parent(view));
                                  ...
                                  ThunarFile   *new_directory)
{
  ...
  for (n = 0; n < n_pages; n++)
    {
      /* get the tab's container and then the view */
      tab_page = gtk_notebook_get_nth_page (GTK_NOTEBOOK (window->notebook_selected), n);
      view = g_object_get_data(G_OBJECT(tab_page), "thunar-view");
                                 ...
                                 GtkWidget    *menu_item)
{
  ...
  ThunarTerminalWidget *terminal_to_move = thunar_window_get_terminal_for_view(window, view_to_move);
  ...
  if (thunar_window_split_view_is_active (window))
    {
      ...
      g_object_unref (notebook_to_close);

      /* Update the window's pointers to reflect the closed pane. */
...
/* Toggle the embedded terminal visibility for current view */
static gboolean
thunar_window_action_view_terminal (ThunarWindow *window,
                                    GtkWidget    *menu_item)
{
                            ...
                            GType         view_type)
{
  ...
  if (!GTK_IS_PANED(paned_container) || !THUNAR_IS_TERMINAL_WIDGET(terminal_to_reuse)) {
      ...
      if (current_directory) g_object_unref(current_directory);
      return;
  }
  ...
  // If the replaced view was the active one, update the window's main view pointer.
                                   ...
                                   gpointer   user_data)
{
  ...
  /* Handle F4 to toggle embedded terminal */
  if (key_event->type == GDK_KEY_PRESS)
    {
      GdkEventKey *evt = (GdkEventKey *) key_event;
      if (evt->keyval == GDK_KEY_F4)
        {
          /* Toggle visibility for the terminal in the active view */
          ThunarTerminalWidget *current_terminal = thunar_window_get_terminal_for_view (thunar_window, thunar_window->view);
          if (current_terminal != NULL)
            {
              thunar_terminal_widget_toggle_visible (current_terminal);
              return GDK_EVENT_STOP;
                                               ...
                                               gboolean      directory_specific_settings)
{
  ...
  for (lp = tabs; lp != NULL; lp = lp->next)
    {
      view = g_object_get_data(G_OBJECT(lp->data), "thunar-view");
      ...
      thunar_window_replace_view (window, view, view_type);
    }
                               ...
                               gint         *active_page)
{
  ...
  for (n = 0; n < n_pages; n++)
    {
      /* get the view from its container */
      tab_page = gtk_notebook_get_nth_page (GTK_NOTEBOOK (window->notebook_selected), n);
      view = g_object_get_data(G_OBJECT(tab_page), "thunar-view");
...
/* Per-tab terminal management functions */

/**
 * thunar_window_get_terminal_for_view:
 * @window: a #ThunarWindow instance
 * @view: the view widget to get the terminal for
 *
 * Helper function to get the terminal for a given view by looking it up
 * in the view_terminals hash table.
 *
 * Returns: The terminal widget for the view, or NULL.
 **/
static ThunarTerminalWidget *
thunar_window_get_terminal_for_view (ThunarWindow *window,
                                     GtkWidget    *view)
...
/**
 * thunar_window_switch_to_view_terminal:
 * @window: a #ThunarWindow instance
 * @view: the view widget to switch terminal to
 *
 * Switches the active terminal pointer to the one associated with the given view.
 * This does NOT change the visibility of any terminal, allowing multiple terminals
 * to be visible at once in split view.
 **/
void
...
/**
 * thunar_window_terminal_directory_changed:
 * @terminal: the terminal widget that changed directory
 * @location: the new directory location
 * @window: the #ThunarWindow instance
 * 
 * Callback for when a terminal changes directory. Updates the file manager
 * to follow the terminal's new directory (Terminal -> File Manager sync).
 **/
static void
thunar_window_terminal_directory_changed (ThunarTerminalWidget *terminal,
                                          GFile                *location,
                                          ThunarWindow         *window)
{
  ...
  /* Convert GFile to ThunarFile and update the view */
  thunar_file = thunar_file_get (location, NULL);
