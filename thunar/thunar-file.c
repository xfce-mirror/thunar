  ...
  if (thunar_file_can_execute (file, &ask_execute))
    {
      ...
      if (response == THUNAR_FILE_ASK_EXECUTE_RESPONSE_RUN_IN_TERMINAL ||
          response == THUNAR_FILE_ASK_EXECUTE_RESPONSE_RUN)
        {
          gboolean run_in_terminal = (response == THUNAR_FILE_ASK_EXECUTE_RESPONSE_RUN_IN_TERMINAL);
          return thunar_file_execute (file, NULL, parent, run_in_terminal, NULL, NULL, error);
        }
