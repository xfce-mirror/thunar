                                                ...
                                                gboolean  show_file_size_binary_format)
{
  ...
  gint               text_list_size     = 0;
  GList             *lp;
  gchar            **text_list          = NULL;
  ...
  g_object_unref (preferences);
  text_list = (gchar **) g_malloc0 (sizeof (gchar **) * 4);
  ...
  if (folder_text == NULL && non_folder_text == NULL)
    text_list[text_list_size++] = g_strdup (_("0 items"));
  if (folder_text != NULL)
    text_list[text_list_size++] = folder_text;
  if (non_folder_text != NULL)
    text_list[text_list_size++] =  non_folder_text;

  if (show_last_modified && last_modified_file != NULL)
    text_list[text_list_size++] = g_strdup_printf (_("Last Modified: %s"), thunar_file_get_date_string (last_modified_file, THUNAR_FILE_DATE_MODIFIED, store->date_style, store->date_custom_style));
  text_list[text_list_size++] = NULL;
  text = g_strjoinv ("|", text_list);
  g_free (text_list);
  return text;
}
                                      ...
                                      GList           *selected_items)
{
  ...
  show_last_modified = thunar_status_bar_info_check_active (active, THUNAR_STATUS_BAR_INFO_LAST_MODIFIED);
  text_list = (gchar **) g_malloc0 (sizeof (gchar **) * 7);
  ...
  if (selected_items == NULL) /* nothing selected */
    {
      ...
      if (G_LIKELY (file != NULL
          && thunar_g_file_get_free_space (thunar_file_get_file (file), &size, NULL)))
        {
          ...
          s = g_strdup_printf (_("Free space: %s"), fspace_string);

          text_list[text_list_size++] = size_string;
          text_list[text_list_size++] = s;

          /* cleanup */
          g_free (fspace_string);
        }
      else
        text_list[text_list_size++] = thunar_list_model_get_statusbar_text_for_files (store, relevant_files, show_file_size_binary_format);
      ...
      if (show_display_name == TRUE)
        {
          ...
          text_list[text_list_size++] = display_name;
        }
      
      if (show_size == TRUE && (thunar_file_is_regular (file) || G_UNLIKELY (thunar_file_is_symlink (file))))
        {
          if (show_size_in_bytes == TRUE)
      ...
      if (show_filetype == TRUE)
        {
          ...
          text_list[text_list_size++] = filetype_string;
        }
      ...
      if (show_last_modified == TRUE)
        {
          ...
          text_list[text_list_size++] = last_modified_string;
          g_free(s);
        }
      ...
      if (G_UNLIKELY (original_path != NULL))
        {
          ...
          original_path_string = g_strdup_printf ("%s %s", _("Original Path:"), 
                                                  g_filename_display_name (original_path));
          text_list[text_list_size++] = original_path_string;
        }
          ...
          if (show_image_size)
            {
              ...
              if (absolute_path != NULL
                  && gdk_pixbuf_get_file_info (absolute_path, &width, &height) != NULL)
                  {
                    ...
                    text_list[text_list_size++] = image_size_string;
                  }
  ...
  else /* more than one item selected */
    {
      ...
      s = thunar_list_model_get_statusbar_text_for_files (store, relevant_files, show_file_size_binary_format);
      if (s[0] != '\0')
        s = g_strdup_printf (_("Selection: %s"), s);
      else
        s = g_strdup ("");
      g_list_free (relevant_files);
      text_list[text_list_size++] =  s;
