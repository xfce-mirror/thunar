                                                ...
                                                gboolean  show_file_size_binary_format)
{
  ...
  if (folder_text == NULL && non_folder_text == NULL)
    text_list = g_list_append (text_list, g_strdup (_("0 items")));
  if (folder_text != NULL)
  ...
  if (show_last_modified && last_modified_file != NULL)
    text_list = g_list_append (text_list,
                               g_strdup_printf (_("Last Modified: %s"), thunar_file_get_date_string (last_modified_file, THUNAR_FILE_DATE_MODIFIED, store->date_style, store->date_custom_style)));
  text_array = (gchar **) g_malloc0 (sizeof (gchar **) * (g_list_length (text_list) + 1));
  for (lp = text_list; lp != NULL; lp = lp->next)
    text_array[text_list_size++] =  lp->data;
  text_array[text_list_size++] = NULL;
  text = g_strjoinv (" | ", text_array);
  ...
  if (selected_items == NULL) /* nothing selected */
    {
      ...
      if (G_LIKELY (file != NULL
        ...
        text_list = g_list_append (text_list, thunar_list_model_get_statusbar_text_for_files (store, relevant_files, show_file_size_binary_format));
