                                      ...
                                      GList           *selected_items)
{
  ...
  GList             *lp;
  GList             *text_list                = NULL;
  gchar             *temp_string_1            = NULL;
  gchar             *temp_string_2            = NULL;
  gchar             *text                     = "";
  gint               height;
  gint               width;
  GSequenceIter     *row;
  ...
  if (selected_items == NULL) /* nothing selected */
    {
      ...
      temp_string_1 = thunar_list_model_get_statusbar_text_for_files (store, relevant_files, show_file_size_binary_format);
      text_list = g_list_append (text_list, temp_string_1);
          ...
          && thunar_g_file_get_free_space (thunar_file_get_file (file), &size, NULL)))
            {
              ...
              temp_string_2 = g_format_size_full (size, show_file_size_binary_format ? G_FORMAT_SIZE_IEC_UNITS : G_FORMAT_SIZE_DEFAULT);
              temp_string_1= g_strdup_printf (_("Free space: %s"), temp_string_2);
              text_list = g_list_append (text_list, temp_string_1);
              g_free(temp_string_2);
            }
  ...
  else if (selected_items->next == NULL) /* only one item selected */
    {
      ...
      if (show_display_name == TRUE)
        {
          temp_string_1 = g_strdup_printf (_("\"%s\""), thunar_file_get_display_name (file));
          text_list = g_list_append (text_list, temp_string_1);
        }
      ...
      if (thunar_file_is_regular (file) || G_UNLIKELY (thunar_file_is_symlink (file)))
        {
          if (show_size == TRUE)
            {
              if (show_size_in_bytes == TRUE)
                temp_string_1 = thunar_file_get_size_string_long (file, show_file_size_binary_format);
              else
                temp_string_1 = thunar_file_get_size_string_formatted (file, show_file_size_binary_format);
              text_list = g_list_append (text_list, temp_string_1);
            }
        }
      ...
      if (G_UNLIKELY (original_path != NULL))
        {
          ...
          temp_string_2 = g_filename_display_name (original_path);
          temp_string_1 = g_strdup_printf (_("%s %s"), "Original Path:", temp_string_2);
          text_list = g_list_append (text_list, temp_string_1);
          g_free (temp_string_2);
        }
          ...
          if (show_image_size)
            {
              ...
              temp_string_2 = g_file_get_path (thunar_file_get_file (file));
              if (temp_string_2 != NULL
                  && gdk_pixbuf_get_file_info (temp_string_2, &width, &height) != NULL)
                  {
                    /* append the image dimensions to the statusbar text */
                    temp_string_1 = g_strdup_printf ("%s %dx%d", _("Image Size:"), width, height);
                    text_list = g_list_append (text_list, temp_string_1);
                  }
      ...
      if (show_last_modified)
        {
          temp_string_2 = thunar_file_get_date_string (file, THUNAR_FILE_DATE_MODIFIED, store->date_style, store->date_custom_style);
          temp_string_1 = g_strdup_printf (_("Last Modified: %s"), temp_string_2);
          text_list = g_list_append (text_list, temp_string_1);
          g_free (temp_string_2);
        }
    }
  else /* more than one item selected */
    {
      ...
      temp_string_1 = g_strdup_printf (_("Selection: %s"),
