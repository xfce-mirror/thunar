...
static void               thunar_list_model_file_changed                (ThunarFolder                 *folder,
                                                                         ThunarFile                   *file,
...
static void               thunar_list_model_search_finished             (ThunarJob                    *job,
                                                                         ThunarListModel              *store);
static void               thunar_list_model_add_search_files (ThunarStandardViewModel *model,
                                                              GList                   *files);
static gboolean           thunar_list_model_update_search_files (ThunarListModel *model);
                             ...
                             GValue       *value)
{
  ...
  if (THUNAR_LIST_MODEL (model)->check_file_in_model_before_use &&
      ...
      THUNAR_LIST_MODEL (model)->file_was_sorted)
    {
...
static void
thunar_list_model_file_changed (ThunarFolder    *folder,
                                ThunarFile      *file,
                                ThunarListModel *store)
{
  GSequenceIter *row;
                                    ...
                                    GList                   *files)
{
  ...
  _model->files_to_add = g_list_concat (_model->files_to_add, files);
  ...
  if (folder != NULL)
    {
      ...
      g_signal_connect (G_OBJECT (store->folder), "file-changed", G_CALLBACK (thunar_list_model_file_changed), store);
                                       ...
                                       gpointer model)
{
  ...
  thunar_list_model_file_changed (store->folder, file, store);
}
