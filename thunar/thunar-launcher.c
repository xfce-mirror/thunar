...
static void                    thunar_launcher_menu_item_activated        (ThunarLauncher                 *launcher,
                                                                           GtkWidget                      *menu_item);
static gchar*                  thunar_generate_incremental_file_name      (ThunarFile                     *dir,
...
static gchar*
thunar_generate_incremental_file_name (ThunarFile *dir, const gchar *file_name)
{
  ...
  for (GList *files = thunar_folder_get_files (folder); files != NULL; files = files->next)
    {
      ...
      if (strncmp (thunar_file_get_display_name (file), file_name, file_name_size) == 0)
        {
          count++;
          if (!found && strcmp (thunar_file_get_display_name (file), file_name) == 0)
            {
              found = TRUE;
            }
        }
  ...
  if (!found)
  ...
  else
    {
      ...
      return g_strconcat (file_name, result, NULL);
    }
}
...
thunar_launcher_action_create_folder (ThunarLauncher *launcher)
{
  ...
  generated_name = thunar_generate_incremental_file_name (launcher->current_directory, _("New Folder")); // does the translation work?
  ...
  if (template_file != NULL)
    {
      ...
      generated_name = thunar_generate_incremental_file_name (launcher->current_directory, thunar_file_get_display_name (template_file));
      name = thunar_dialogs_show_create (launcher->widget,
  ...
  else
    {
      ...
      generated_name = thunar_generate_incremental_file_name (launcher->current_directory, "New Empty File");
      name = thunar_dialogs_show_create (launcher->widget,
                                         "text/plain",
                                         generated_name,
                                         _("New Empty File..."));
