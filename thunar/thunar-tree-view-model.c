  ...
  if (!thunar_folder_get_loading (dir) && !node->loaded)
    {
      ...
      g_object_notify (G_OBJECT (node->model), "loading");
    }
}
...
thunar_tree_view_model_load_dir (Node *node)
{
  ...
  if (!thunar_tree_view_model_get_loading (node->model))
  {
  ...
  if (node->dir != NULL)
    {
      ...
      g_assert (g_hash_table_contains (node->model->subdirs, node->file));
