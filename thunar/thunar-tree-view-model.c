...
#define THUNAR_WARN_RETURN(expr, val) \
if (expr) {            \
  g_warn_if_reached(); \
  return val;          \
}
...
static gboolean          thunar_tree_view_model_iter_next (GtkTreeModel *model,
                                                           GtkTreeIter  *iter);
static gboolean          thunar_tree_view_model_iter_previous (GtkTreeModel *model,
                                                               GtkTreeIter  *iter);
...
static void              thunar_tree_view_model_dir_remove_file (Node       *node,
                                                                 ThunarFile *file);
static Node             *thunar_tree_view_model_locate_file (ThunarTreeViewModel *model,
                                                             ThunarFile          *file);
                                 ...
                                 GtkTreeIter  *iter)
{
  ...
  THUNAR_WARN_RETURN (g_sequence_iter_is_end (iter->user_data), NULL);
                                  ...
                                  GtkTreeIter  *iter)
{
  ...
  THUNAR_WARN_RETURN (g_sequence_iter_is_end (iter->user_data), FALSE);
                                      ...
                                      GtkTreeIter  *iter)
{
  ...
  THUNAR_WARN_RETURN (g_sequence_iter_is_begin (iter->user_data), FALSE);
                                      ...
                                      GtkTreeIter  *parent)
{
  ...
  THUNAR_WARN_RETURN (g_sequence_iter_is_end (ptr), FALSE);
...
thunar_tree_view_model_new_node (ThunarFile *file)
{
  ...
  THUNAR_WARN_RETURN (file == NULL, NULL);
  ...
  if (pos_before != pos_after)
    {
      ...
      if (node->parent->ptr != NULL)
        {
          GTK_TREE_ITER_INIT (tree_iter, model->stamp, node->parent->ptr);
          path = gtk_tree_model_get_path (GTK_TREE_MODEL (model), &tree_iter);
