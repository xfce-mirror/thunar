...
{
    { THUNAR_DETAILS_VIEW_ACTION_CONFIGURE_COLUMNS,           "<Actions>/ThunarStandardView/configure-columns",         "", XFCE_GTK_MENU_ITEM ,       N_ ("Configure _Columns..."),      N_ ("Configure the columns in the detailed list view"), NULL, G_CALLBACK (thunar_column_order_editor_new_and_show),       },
      ...
      if (G_UNLIKELY (column == THUNAR_COLUMN_NAME))
        {
          ...
          g_object_set (G_OBJECT (THUNAR_STANDARD_VIEW (details_view)->name_renderer),
                        "xalign", 0.0, "ellipsize", PANGO_ELLIPSIZE_MIDDLE, "width-chars", 60, NULL);
...
thunar_details_view_realize (GtkWidget *widget)
{
  ...
  /* Needs to be done on realize, because the binding on 'scroll' only will take effect after realize */
...
thunar_details_view_get_visible_files (ThunarDetailsView *details_view)
{
  ...
  if (gtk_tree_model_get_iter (GTK_TREE_MODEL (model), &start_iter, start_path) && gtk_tree_model_get_iter (GTK_TREE_MODEL (model), &end_iter, end_path))
                                               ...
                                               ThunarDetailsView *details_view)
{
  ...
  _thunar_return_val_if_fail (GTK_IS_TREE_VIEW (tree_view), NULL);
  _thunar_return_val_if_fail (THUNAR_IS_DETAILS_VIEW (details_view), NULL);
...
thunar_details_view_update_visible_expand_arrows (ThunarDetailsView *details_view)
{
  ...
  /* Reset wait time if the timer is already running */
  if (details_view->update_expand_arrows_timeout_source_id != 0)
    g_source_remove (details_view->update_expand_arrows_timeout_source_id);
                                            ...
                                            gboolean           expandable_folders)
{
  ...
  if (details_view->expandable_folders)
    {
      /* Note: This binding activates on 'scroll'. It will only work when done after the details view got realized */
