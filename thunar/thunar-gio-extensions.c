                                    ...
                                    size_t       *buf_size_out)
{
  ...
  // for direct I/O (stx_dio_mem_align), as well as what the buffer size is
  // required to be a multiple of (stx_dio_offset_align).
  DBG ("Trying statx ()");
      ...
      if (statx_buf.stx_dio_mem_align > 0 && statx_buf.stx_dio_offset_align > 0)
        {
          ...
          // operations.  If somehow the required alignment/offset alignment is
          // lower than 16, it's always safe to go higher.
  ...
  if (buf_align == 0 || offset_align == 0)
    {
      // If either of these are zero, statx () reports that direct I/O is not
      // supported on file's device.
      goto out_err;
    }

  if ((offset_align & (offset_align - 1)) == 0)
    {
      // offset_align is a power of two, so it's easy to "round" CMF_BUF_SIZE
      // down to the nearest multiple of offset_align.
      buf_size = (CMP_BUF_SIZE) & ~(offset_align - 1);
    }
  else
    {
      // I wouldn't expct this to ever not be a power of two, but we can still
      // do something useful here to get a multiple of offset_align.
  ...
  // Create buffers to store the read data, with the required alignment and
  // size.  We create both the source adn dest buffers here since they need to
  // be the same size.
  ...
  // allocated our buffers, it's still possible that there's something wrong
  // with our buffer size or alignment.  So do a test read from the beginning
  // of the file to see if it succeeds, and if so, seek back to the beginning
  // of the file.
...
static GInputStream *
open_file_and_buffer_for_simd (GFile        *file,
                               GCancellable *cancellable,
                               void        **buf_out,
                               unsigned int *buf_align_out,
                               size_t       *buf_size_out,
  ...
  if (inp_a != NULL)
    {
      ...
      if (inp_b == NULL)
        {
          inp_b = open_file_and_buffer_for_simd (file_b, cancellable, &buf_b, &buf_align, &buf_size, error);
        }
    }
