                       ...
                       GError   **error)
{
  ...
  for (lp = base_file_list;
       err == NULL && lp != NULL && !thunar_job_is_cancelled (THUNAR_JOB (job));
       lp = lp->next)
    {
  ...
  if (template_file != NULL)
    {
      ...
      template_stream = g_file_read (template_file, thunar_job_get_cancellable (THUNAR_JOB (job)), &err);
...
again:
      ...
      if (!g_file_make_directory (lp->data, thunar_job_get_cancellable (THUNAR_JOB (job)), &err))
        {
          if (err->code == G_IO_ERROR_EXISTS)
            {
              g_error_free (err);
                        ...
                        GError   **error)
{
  ...
  for (lp = file_list;
       lp != NULL && !thunar_job_is_cancelled (THUNAR_JOB (job));
       lp = lp->next, n_processed++)
    {
      g_assert (G_IS_FILE (lp->data));
                    ...
                    GError   **error)
{
  ...
  else if (thunar_job_set_error_if_cancelled (THUNAR_JOB (job), &err))
    {
      g_propagate_error (error, err);
      return FALSE;
    }
                        ...
                        GError   **error)
{
  ...
  if (thunar_file_rename (file, display_name, thunar_job_get_cancellable (THUNAR_JOB (job)), TRUE, &err))
    {
      thunar_job_send_to_mainloop (THUNAR_JOB (job),
                       ...
                       gboolean                       show_hidden)
{
  ...
  while (thunar_job_is_cancelled (THUNAR_JOB (job)) == FALSE)
    {
      GFile     *file;
      GFileInfo *info;
      GFileType  type;
  ...
  while (g_hash_table_iter_next (&iter_map, &thunar_file, &value))
    {
      ...
      /* Update the boolean value in the hashtable of the caller */
...
{
  /* Use a table of [ThunarFile, GFile] while processing the job, because ThunarFile is not thread-save */
  GHashTable    *g_files = g_hash_table_new_full (g_direct_hash, NULL, g_object_unref, g_object_unref);
  GHashTableIter iter;
  gpointer       key;

  g_hash_table_iter_init (&iter, map);
  while (g_hash_table_iter_next (&iter, &key, NULL))
    g_hash_table_insert (g_files, g_object_ref (key), g_object_ref (thunar_file_get_file (THUNAR_FILE (key))));

  ThunarJob *job = thunar_simple_job_new (_thunar_job_check_empty, 2,
                                          G_TYPE_POINTER, map,
                                          G_TYPE_POINTER, g_files);
