...
//  gtk_widget_add_events (GTK_WIDGET (statusbar), GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);

  statusbar->preferences = thunar_preferences_get (); // TODO: unref
}



void
thunar_statusbar_setup_event (ThunarStatusbar *status_bar,
                              GtkWidget       *event_box)
{
  g_signal_connect (G_OBJECT (event_box), "button-press-event", G_CALLBACK (thunar_statusbar_button_press_event), G_OBJECT (status_bar));
}
...
thunar_statusbar_context_menu (ThunarStatusbar *statusbar)
{
  ...
  g_object_get (G_OBJECT (statusbar->preferences),
                "misc-status-bar-show-size", &show_size,
                "misc-status-bar-show-size-in-bytes", &show_size_in_bytes,
                "misc-status-bar-show-filetype", &show_filetype,
                "misc-status-bar-show-display-name", &show_display_name,
                NULL);
  ...
  xfce_gtk_toggle_menu_item_new_from_action_entry (get_action_entry (THUNAR_STATUS_BAR_ACTION_TOGGLE_SIZE_IN_BYTES), G_OBJECT (statusbar), show_size_in_bytes, GTK_MENU_SHELL (context_menu));
...
thunar_statusbar_action_show_size (ThunarStatusbar *statusbar)
{
  ...
  g_object_get (G_OBJECT (statusbar->preferences), "misc-status-bar-show-size", &active, NULL);
  g_object_set (G_OBJECT (statusbar->preferences), "misc-status-bar-show-size", !active, NULL);
}
...
thunar_statusbar_action_show_size_bytes (ThunarStatusbar *statusbar)
{
  ...
  g_object_get (G_OBJECT (statusbar->preferences), "misc-status-bar-show-size-in-bytes", &active, NULL);
  g_object_set (G_OBJECT (statusbar->preferences), "misc-status-bar-show-size-in-bytes", !active, NULL);
}
...
thunar_statusbar_action_show_filetype (ThunarStatusbar *statusbar)
{
  ...
  g_object_get (G_OBJECT (statusbar->preferences), "misc-status-bar-show-filetype", &active, NULL);
  g_object_set (G_OBJECT (statusbar->preferences), "misc-status-bar-show-filetype", !active, NULL);
}
...
thunar_statusbar_action_show_display_name (ThunarStatusbar *statusbar)
{
  ...
  g_object_get (G_OBJECT (statusbar->preferences), "misc-status-bar-show-display-name", &active, NULL);
  g_object_set (G_OBJECT (statusbar->preferences), "misc-status-bar-show-display-name", !active, NULL);
}
