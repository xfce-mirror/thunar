...
thunar_icon_renderer_class_init (ThunarIconRendererClass *klass)
{
  ...
  /**
   * ThunarIconRenderer:square-icons:
   *
   * Determines whether the icons generated by the renderer are squared or not.
   **/
  g_object_class_install_property (gobject_class,
                                   PROP_SQUARE_ICONS,
                                   g_param_spec_boolean ("square-icons", "square-icons", "square-icons",
                                                         FALSE,
                                                         EXO_PARAM_READWRITE));
}
                                   ...
                                   GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SQUARE_ICONS:
      g_value_set_boolean (value, icon_renderer->square_icons);
      break;
                                   ...
                                   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SQUARE_ICONS:
      icon_renderer->square_icons = g_value_get_boolean (value);
      break;
                             ...
                             GtkCellRendererState flags)
{
  ...
  g_object_get (icon_renderer, "square-icons", &square_icons, NULL);
  ...
  if (square_icons)
    {
      ...
      temp = gdk_pixbuf_new_subpixbuf (icon,
                                       (icon_area.width - smaller_dimension) / 2,
                                       (icon_area.height - smaller_dimension) / 2,
                                       smaller_dimension,
                                       smaller_dimension);
      g_object_unref (icon);
      icon = temp;

      /* then scale it up to the size of the cell */
      temp = gdk_pixbuf_scale_simple (icon, cell_area->width, cell_area->height, GDK_INTERP_BILINEAR);
      g_object_unref (icon);
      icon = temp;
